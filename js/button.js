...
$(document).ready(function() {
	...
	$('button').click(function(){
		...
		$.get('http://gateway-a.watsonplatform.net/calls/url/URLGetRankedKeywords?apikey=909d2935c04ba8e5001c01e3c1c183d64e0de728&url='+ newURL +'&outputMode=json', function(data){
			...
			$('.container').append("<div class='panel panel-default'>" + "<div class='panel-heading'>" + "<h3 class='panel-title'>" + newURL + "</h3>" + "</div>" + time + "<div class='panel-body'>" + keywordlist + "</div>" + "</div>" + "</div>");
			//$('.card').draggable();
		});
	});
});
